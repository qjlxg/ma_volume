...
def process_file(file_path):
    ...
    假设 CSV 文件包含: Date, Open, High, Low, Close, Volume
    """
    try:
        ...
        # 优化：只读取最后一行，但为了确保数据完整性，通常需要读取至少一行
        # 这里读取整个文件，然后取最后一行
...
def main():
    ...
    # 3. 使用多进程并行处理
    # 核心数为-1表示使用所有可用的CPU核心
    with Pool(cpu_count()) as pool:
        # map() 函数将 process_file 应用于 all_files 中的每个元素
        # 它会阻塞直到所有结果都可用
        results = pool.map(process_file, all_files)

    # 4. 收集和整理结果
    # 过滤掉 None 值，得到符合条件的股票代码列表
    ...
    # 结果文件名：result_YYYYMMDD_HHMMSS.csv
